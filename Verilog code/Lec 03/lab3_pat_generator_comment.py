...
def sudoku_generator(illegal) :
    rBase = range(BASE)
    rows = [ g*BASE + r for g in shuffle(rBase) for r in shuffle(rBase) ] #2*3+2=8, rows range is 0~8
    if illegal==1 :
        rows = shuffle(rows)
    cols = [ g*BASE + c for g in shuffle(rBase) for c in shuffle(rBase) ] #2*3+2=8, rows range is 0~8
    ...
    for n in range(1, 10) :
        if is_legal(board, r, c, n)==True :
            ...
            if dfs(board, blanks, i+1 )==True :
                return True
            #if not found the matched number make baord[r][c] = 0
            board[r][c] = 0
    #if board[r][c] was active then return def return False.
    return False
